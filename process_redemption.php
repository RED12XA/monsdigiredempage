...
ini_set('display_errors', 0); // Change to 1 if testing locally
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        // Basic validation
        if (empty($order_id)) $response['errors'][] = "Order ID is required";
        if (empty($redeem_code)) $response['errors'][] = "Redemption code is required";
        if (empty($email)) {
        ...
        if (empty($response['errors'])) {
            ...
            // Confirm table exists (remove in prod)
            $debug_stmt = $pdo->query("SHOW TABLES LIKE 'redemption_codes'");
            if ($debug_stmt->rowCount() === 0) {
                throw new Exception("Redemption codes table does not exist.");
            }

            // Check if code + order_id exists
            $stmt = $pdo->prepare("SELECT * FROM redemption_codes WHERE code = ? AND order_id = ? LIMIT 1");
            $stmt->execute([$redeem_code, $order_id]);
            $redemption = $stmt->fetch();

            if ($redemption) {
                // Check if the redemption code has already been redeemed
                if ($redemption['is_redeemed'] == 1) {
                    ...
                    $product_stmt->bindParam(':product_id', $redemption['product_id'], PDO::PARAM_INT);
                    $product_stmt->execute();
                    $product = $product_stmt->fetch();

                    // Check if product was found
                    if ($product) {
                        // Mark the redemption code as redeemed and store the user's email
                        ...
                        // Send the product details via email
                        if (sendProductEmail($email, $product)) {
                            $response['success'] = true;
                            $response['message'] = "Your code has been redeemed successfully. Product sent to your email.";
                        } else {
                            throw new Exception("Failed to send product email.");
                        }
                    } else {
                ...
                $response['message'] = "Invalid redemption code or order ID.";
            }
        } else {
        ...
        $response['debug'] = $e->getMessage(); // Remove or comment this in production
    } catch (Exception $e) {
